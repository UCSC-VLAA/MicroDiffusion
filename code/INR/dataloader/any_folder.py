...
def load_imgs(image_dir, num_img_to_load, start, end, skip, load_sorted, load_img, path= r"./neurons.npy"):
    ...
    datafolder = '.'
    #path = r'./case1_50um.npy'
    # path = r"./GT3d_inXYZ.npy"
    img_list = np.load(path)
    print(img_list.shape)

    # img_list = np.()
    # sio.loadmat(dir)['rawData']
    
    # print(sio.loadmat(dir))
    H,W,N_imgs = img_list.shape

    img_list = img_list.reshape(H,W,N_imgs,1).astype(np.float32)
    img_list = torch.tensor(img_list).cuda().permute(2,0,1,3)
    print(img_list.shape)

    img_list = resize_imgs(img_list,128,128).permute(1,2,0,3).cpu().numpy()
    print(img_list.shape)
    H,W,N_imgs,C = img_list.shape
    ...
    img_list = torch.from_numpy(img_list).float()  # (N, H, W, 3) torch.float32
    print("list",img_list.shape)
    # print(img_list)
    ...
    #     for p in img_paths:
    #         img = imageio.imread(p,as_gray=True)
...
class DataLoaderAnyFolder:
    ...
    Most useful fields:
        ...
        self.N_imgs         scalar
    """
    def __init__(self, base_dir, scene_name, res_ratio, num_img_to_load, start, end, skip, load_sorted, path,load_img=True):
        """
        :param base_dir:
        :param scene_name:
