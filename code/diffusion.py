...
class GaussianDiffusion(nn.Module):
    ...
    def q_sample(self, x_0:torch.Tensor, t:torch.Tensor):
        ...
        return self._extract(self.sqrt_alphas_bar, t, x_0.shape) * x_0 + self._extract(self.sqrt_one_minus_alphas_bar, t, x_0.shape) * eps, eps
    ...
    def sample(self, shape:tuple,inter, **model_kwargs) -> torch.Tensor:
        """
    ...
    def trainloss(self, x_inter ,x_0:torch.Tensor, cembs) -> torch.Tensor:
        """
        ...
        x_inter = torch.mean(torch.stack(x_inter).squeeze(dim=1),dim=0).unsqueeze(0)
        cemb = torch.mean(torch.stack(cembs).squeeze(dim=1),dim=0).unsqueeze(0)  
        x_0 = 0.1*x_0 + 0.9*x_inter
        x_t, eps = self.q_sample(x_0, t)
        pred_eps = self.model(x_t, t,cemb=cemb)
        eps_total.append(eps)
        pred_eps_total.append(pred_eps)

        pred_eps_total = torch.stack(pred_eps_total,dim=0).to(x_0.device)
        eps_total = torch.stack(eps_total,dim=0).to(x_0.device)
        
        pred_eps_mean = torch.mean(pred_eps_total,dim=0).squeeze()
        eps_mean = torch.mean(eps_total,dim=0).squeeze()
        
        loss = F.mse_loss(pred_eps_mean, eps_mean, reduction='mean')
        return loss, pred_eps_total
